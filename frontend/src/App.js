...
class App extends Component{
  ...
  refreshList= () => {
    axios.get("http://localhost:8000/api/tasks/").then(
      res => this.setState({todoList: res.data})).catch(err => console.log(err));
  };
 ...
 render(){
  return(
    <main className='content p-3 mb-2 bg-primary'>
      ...
      <div className='row'>
        <div className='col-md-6 col-sma-10 mx-auto p-0'>
          ...
          {this.renderTabList()}
          <ul className='list-group list-group-flusk'>
            {this.renderItems()}
          </ul>
        </div>
        </div>
      </div>
      <footer className='my-3 mb-2 bg-primary text-white text-center'>Copyright 2024 &copy; All Rights Reserved</footer>
      {this.state.modal ?(
